...
export const getProfileSettings = async (id: number) => {
    try {
        // Выполнение POST запроса для регистрации
        const response = await axios.get(`${OUR_API_ADDRESS}/${OUR_API_ENDPOINTS.profile}/${OUR_API_ENDPOINTS.settings}/${id}`);
        return response.data;
    } catch (error: any) {
        // Обработка ошибок регистрации
        if (error.response && error.response.data && error.response.data.message) {
...
export const putProfileSettings = async (data: {
    ...
    try {
        // Выполнение POST запроса для регистрации
        const response = await axios.put(`${OUR_API_ADDRESS}/${OUR_API_ENDPOINTS.profile}/${OUR_API_ENDPOINTS.settings}`, data);
        return response.data;
    } catch (error: any) {
        // Обработка ошибок регистрации
        if (error.response && error.response.data && error.response.data.message) {
